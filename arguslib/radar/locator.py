...
class RadarData:
    ...
    def _get_gridded_rhi_data(self, dt, var) -> xr.DataArray:
        ...
        if var not in radar.fields:
            raise ValueError(f"Variable {var} not found in radar file: {self.current_file_path}")
    ...
    def get_pyart_radar(self, dt) -> pyart.core.Radar:
        ...
        if self.radar_data and self.current_file_path:
            try:
                if hasattr(self.radar_data, 'time') and \
                   'data' in self.radar_data.time and \
                   self.radar_data.time['data'].size > 0:
                    ...
                    scan_duration_seconds = self.radar_data.time['data'][-1] - self.radar_data.time['data'][0]
                    scan_end_time = scan_start_time + datetime.timedelta(seconds=scan_duration_seconds)
        ...
        if filepath is None:
            raise FileNotFoundError(f"No radar file found containing the time {dt} for campaign {self.campaign}, scan_type {self.scan_type}")
        ...
        if self.radar_data: # Ensure radar_data is loaded
            scan_start_time = pyart.util.datetime_from_radar(self.radar_data)
            if hasattr(self.radar_data, 'time') and 'data' in self.radar_data.time and self.radar_data.time['data'].size > 0:
                scan_duration_seconds = self.radar_data.time['data'][-1] - self.radar_data.time['data'][0]
                scan_end_time = scan_start_time + datetime.timedelta(seconds=scan_duration_seconds)
                if not (scan_start_time <= dt <= scan_end_time):
                 ...
                 raise ValueError(f"Loaded radar file {filepath} has no valid time data.")
    ...
    def get_next_time(self, dt, max_gap_hrs=48):
        ...
        for f_path in files:
            try:
                ...
                match = re.search(r'(\d{4}\d{2}\d{2}-\d{2}\d{2}\d{2})', filename_base)
                if match:
                    file_start_dt = datetime.datetime.strptime(match.group(1), "%Y%m%d-%H%M%S")
        ...
        start_of_next_day = (dt + datetime.timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
