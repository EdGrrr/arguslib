...
class DirectCamera(Camera):
    ...
    def _show(
        ...
        **kwargs,
        ...
        self.data_loader.image = np.clip(np.uint16(img) * brightness_adjust, 0, 255).astype(np.uint8)
    ...
    def image(self):
        ...
        return Image.fromarray(self.to_image_array()) # Watch out! this image looks like the mp4 files. vertical flip to get it as matplotlib might plot.
    
    def to_image_array(self, time=True):
        import cv2
        slicer = (slice(None, None, -1), slice(None, None, None if self.direct_lr_flip else -1), slice(None, None, None))
        im = self.data_loader.image
        if time:
            cv2.putText(im, self.data_loader.current_image_time.isoformat(timespec='seconds'), (50,2900), 1, 10, (255,200,200), 20, bottomLeftOrigin=True)
