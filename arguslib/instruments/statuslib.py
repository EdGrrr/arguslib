...
from arguslib import geo
from instruments import Position

cameras = {
    "3-7": Camera.from_filename(
        "cam1_calibration.yml",
        Position(-1.439252, 51.146668, 0.1),
        rotation=10,
        scale_factor=3040 / 500,
        camera_type="allsky",
    ),
    "3-8": Camera.from_filename(
        "cam1_calibration.yml",
...
radars = {
    "kepler": Radar(
        0.6, Position(-1.439339, 51.144752, 0.1), rotation=0  # beamwidth in degrees
    )
}
    ...
    for acft in aircraft["aircraft"]:
        try:
            if acft["alt_geom"] < 26000:
                continue
            dist = geo.haversine(
                camera.position.lon, camera.position.lat, acft["lon"], acft["lat"]
            )
            acft_out[acft["hex"]] = {
                "acft_dist": dist,
                "alt": acft["alt_geom"],
            ...
            dist = geo.haversine(
                camera.position.lon, camera.position.lat, track_pos[0], track_pos[1]
            )
            acft_out[acft["hex"]]["track_px"] = pl_track.T

            spread_velocity = (
                3  # Spreading velocity in ms-1 (https://doi.org/10.1029/98JD02594)
            )
            spread_direction = (
                acft["track"] + 90
            ...
            track_leftoffset_km = (
                np.array([acft_u - wind_u + spread_u, acft_v - wind_v + spread_v])[
                    :, None
                ]
                * times
                / 1000
            ...
            track_leftpos = np.array(
                geo.xy_offset_to_ll(acft["lon"], acft["lat"], *track_leftoffset_km)
            )
            pl_track = np.array(
                [
                    camera.target_pix(
            ...
            track_rightpos = np.array(
                geo.xy_offset_to_ll(acft["lon"], acft["lat"], *track_rightoffset_km)
            )
            pl_track = np.array(
                [
                    camera.target_pix(
