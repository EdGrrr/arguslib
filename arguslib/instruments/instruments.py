...
class Position():
    ...
    def target_ead(self, target_position):
        ...
        distance = haversine(self.lon, self.lat, target_position.lon, target_position.lat)
        ...
        target_azimuth = bearing(self.lon, self.lat, target_position.lon, target_position.lat)
    ...
    def target_xyz(self, target_position):
        ...
        target_bearing = bearing(self.lon, self.lat, target_position.lon, target_position.lat)
    ...
    def ead_to_lla(self, target_elevation, target_azimuth, target_distance):
        return self.xyz_to_lla(*self.ead_to_xyz(target_elevation, target_azimuth, target_distance))

    def xyz_to_ead(self, target_x, target_y, target_z):
        alt_diff = target_position.alt-self.alt
        target_distance = np.sqrt(target_x**2+target_y**2+target_z**2)
        target_elevation = np.rad2deg(np.arctan2(target_z, np.sqrt(target_x**2+target_y**2)))
...
class Camera(Instrument):
    def __init__(self, intrinsic_calibration: Projection,
                 *args,
                 scale_factor = 1,
                 camera_type='allsky',
                 **kwargs):
    ...
    def from_filename(cls, filename,
                      *args,
                      **kwargs):
        return cls(Projection.fromfile(filename),
                   *args, **kwargs)
    ...
    def target_pix(self, target_position: Position):
        return self.iead_to_pix(
            *self.target_iead(target_position))

    def pix_to_iead(self, pix_x, pix_y, distance):
        xyz = self.intrinsic.image_to_view([
            pix_x*self.scale_factor,
            pix_y*self.scale_factor])
    ...
    def iead_to_pix(self, elevation, azimuth, dist=10):
        return self.intrinsic.view_to_image(
            self.position.ead_to_xyz(elevation, azimuth, dist))/self.scale_factor
    
    def radar_beam(self, target_elevation, target_azimuth, radar):
        output = []
        for beam_r in range(0, 361, 30):
            output.append(self.el_azi_to_pixel(
        ...
        if self.beamwidth:
            return np.array([
                [self.iead_to_lla(radar_elevation, radar_azimuth, radar_dist),
                 self.iead_to_lla(radar_elevation+self.beamwidth/2, radar_azimuth, radar_dist),
                 self.iead_to_lla(radar_elevation, radar_azimuth+self.beamwidth/2, radar_dist),
                 self.iead_to_lla(radar_elevation-self.beamwidth/2, radar_azimuth, radar_dist),
                 self.iead_to_lla(radar_elevation, radar_azimuth-self.beamwidth/2, radar_dist)]
