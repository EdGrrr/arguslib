...
cam1 = Camera.from_filename('cam1_calibration.yml', cam1_position, rotation=10, scale_factor=3040/500)
...
# Get aircraft data
response = requests.get(f'https://opendata.adsb.fi/api/v2/lat/{cam1_position.lat}/lon/{cam1_position.lon}/dist/100')
...
for acft in aircraft['aircraft']:
    try:
        if acft['alt_geom']<26000:
            continue
        dist = geo.haversine(cam1.position.lon, cam1.position.lat, acft['lon'], acft['lat'])
        acft_out[acft['hex']] = {
            'acft_dist': dist,
            'flight': acft['flight'],
            't': acft['t']}
        ...
        if dist < 90:
            pl = cam1.target_pix(Position(acft['lon'], acft['lat'], acft['alt_geom']/(3.33*1000)))
        ...
        pl_track = np.array([cam1.target_pix(Position(tp[0], tp[1], acft['alt_geom']/(3.33*1000)))
                             for tp in track_pos.T])

        dist = geo.haversine(cam1.position.lon, cam1.position.lat, track_pos[0], track_pos[1])
