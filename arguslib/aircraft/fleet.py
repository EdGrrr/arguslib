...
from csat2.ECMWF import ERA5WindData3D

ERA5_LEVELS = ["150hPa", "175hPa", "200hPa", "225hPa", "250hPa", "300hPa", "350hPa"]


def assign_era5_winds_to_flightlocs(
    flightlocs, base_date, levels=ERA5_LEVELS, res="0.25grid"
):
    """Overwrite flightlocs.data['wind_u'/'wind_v'] with ERA5 winds.

    TrackerLocsFixed reads its winds off the flightlocs object, so this is
    where the source has to be swapped. base_date must be UTC midnight.
    """
    winds = ERA5WindData3D(levels=levels, res=res, linear_interp="space")
    level_vals = [float(l.removesuffix("hPa")) for l in levels]

    data = flightlocs.data
    lon = np.ma.filled(data["lon"], np.nan).astype(float)
    lat = np.ma.filled(data["lat"], np.nan).astype(float)
    alt = np.ma.filled(data["alt_geom"], np.nan).astype(float)

    # Column i is UTC second i*15 (the BST offset slicing in _readin_flights
    # already shifted this), but lon has fewer columns than data['times'].
    secs = np.arange(lon.shape[1]) * flightlocs.time_resolution

    plevel = np.clip(geo.ft_to_hPa(alt), min(level_vals), max(level_vals))
    valid = np.isfinite(lon) & np.isfinite(lat) & np.isfinite(plevel)

    u = np.full(lon.shape, np.nan)
    v = np.full(lon.shape, np.nan)

    # ERA5 is hourly; chunk by hour and interpolate in time between the
    # bracketing fields so points aren't all snapped to one timestep.
    hours = secs // 3600
    for h in np.unique(hours):
        mask = valid & (hours == h)[None, :]
        if not mask.any():
            continue
        ac_i, t_i = np.where(mask)
        t0 = base_date + datetime.timedelta(hours=int(h))
        t1 = t0 + datetime.timedelta(hours=1)
        w = (secs[t_i] - h * 3600) / 3600.0

        args = (lon[ac_i, t_i], lat[ac_i, t_i], plevel[ac_i, t_i])
        u0, v0 = winds.get_data(*args, t0, simple=False)
        u1, v1 = winds.get_data(*args, t1, simple=False)

        u[ac_i, t_i] = (1 - w) * u0 + w * u1
        v[ac_i, t_i] = (1 - w) * v0 + w * v1
...
class Fleet:
    ...
    def set_winds(self, winds):
        """Change the advection wind source. Rebuilds the tracker, since the
        winds are baked in at construction time."""
        if winds not in ("aircraft", "era5"):
            raise ValueError(f"winds must be 'aircraft' or 'era5', got {winds!r}")
        if winds == self.winds:
            return
        self.winds = winds
        if self.loaded_file:
            self.load_output(self.loaded_file, force_reload=True)
    ...
    def load_output(self, filename, force_reload=False, wind_filter=None):
        ...
        base_date = datetime.datetime(year, mon, day)  # before date_to_doy
    ...
    def get_trails_arr(
        ...
        if winds is not None and winds != self.winds:
            raise ValueError(
                f"Fleet was built with winds={self.winds!r}. Advection happens when "
