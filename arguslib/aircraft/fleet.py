...
class AircraftPos():
    def __init__(self, time_resolution=15, variables=['lon', 'lat', 'alt', 'geom']):
        ...
        self.positions = np.full((60*60*24//time_resolution, len(self.variables)), np.nan)
    ...
    def add_position(self, dtime, aircraft_data):
        ...
        add_index = (dtime.hour*3600+dtime.minute*60+dtime.second)//self.time_resolution
        self.positions[add_index] = [aircraft_data.get(name, np.nan) for name in self.variables]
    ...
    def get_current(self, dtime, vname=None):
        ...
        if vname is None:
            ...
            return {vn: jsonfloat(tempdata[self.variables.index(vn)])
    ...
    def get_trail(self, dtime, tlen=2*60*60, spread_velocity=-1, wind_filter=-1):
        ...
        if spread_velocity>0:
            ...
            spread_direction = track+90 # Simple for now - mostly governed by aircraft speed/direction
            spread_u, spread_v = spread_velocity*(np.array([np.sin(np.deg2rad(spread_direction)),
                                                            np.cos(np.deg2rad(spread_direction)),]))
            track_leftoffset_km = track_offset_km + np.array([spread_u, spread_v]) * times /1000
            track_leftpos = np.array(geo.xy_offset_to_ll(lon, lat, *track_leftoffset_km))
            track_rightoffset_km = track_offset_km - np.array([spread_u, spread_v]) * times /1000
            track_rightpos = np.array(geo.xy_offset_to_ll(lon, lat, *track_rightoffset_km))
    ...
    def get_data(self, dtime, vname, tlen=2*60*60):
        ...
        if type(vname) == type('str'):
            vname = [vname]
        return {
            vn: self.positions[startind:index, self.variables.index(vn)]
            for vn in vname}


class Aircraft():
    def __init__(self, icao24, atype=None, time_resolution=15, variables=['lon', 'lat','alt_geom']):
        self.icao24 = icao24
        self.atype = atype
...
class Fleet():
        ...
        for icao24 in acdata.keys():
            try:
                ...
                self.aircraft[icao24] = Aircraft(
                    icao24, acdata[icao24][0],
                    time_resolution=self.time_resolution, variables=self.variables)
                self.aircraft[icao24].add_position(dtime, acdata[icao24][1])
        self.last_update = dtime
    ...
    def __repr__(self):
        return f'Fleet: {len(self.aircraft.keys())}' + f' aircraft, {np.sum([self.aircraft[ac].valid_points() for ac in self.aircraft.keys()])} points'
    ...
    def write_output(self, filename):
        ...
        with netCDF4.Dataset(filename+'.nc', 'w', format='NETCDF4') as ncdf:
            ...
            for vind, vname in enumerate(self.variables):
                ...
                output = np.array([self.aircraft[acft].pos.positions[datastart:, vind] for acft in aircraft_list])
    ...
    def get_trails(self, dtime, tlen=2*60*60, spread_velocity=-1, wind_filter=-1):
        ...
        trails = {}
        for ac in self.aircraft.keys():
            trails[ac] = self.aircraft[ac].get_trail(dtime, tlen, spread_velocity=spread_velocity, wind_filter=wind_filter)
        return trails
