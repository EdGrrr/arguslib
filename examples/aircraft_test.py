...
def get_pcl_details(fleet, acft, dtime, seconds_since_formation):
    ...
    metadata = fleet.aircraft[acft].get_data(
        dtime, ["alt_geom", "oat"], tlen=seconds_since_formation
    )
...
def calc_ead(camera, acft_data):
    return camera.position.target_ead(
        Position(
            acft_data["lon"], acft_data["lat"], acft_data["alt_geom"] / (3.33 * 1000)
        )
    )
...
time_range = [datetime.datetime(2025, 3, 5, 12), datetime.datetime(2025, 3, 5, 14)]
track_step = 60
acft = "405f13"
...
while dtime < time_range[1]:
    if dtime > contrail_form_time + datetime.timedelta(seconds=60):
        acft_data = get_pcl_details(
            fleet, acft, dtime, (dtime - contrail_form_time).total_seconds()
        )
...
output = {v: [output[a][v] for a in range(len(output))] for v in output[0].keys()}

kepler_commands = np.array(
    [
        [
            (
                datetime.datetime(2025, 3, 5, 12, 44, 0) - contrail_form_time
            ).total_seconds(),
            266,
            66,
            51.14,
        ...
        [
            (
                datetime.datetime(2025, 3, 5, 12, 49, 0) - contrail_form_time
            ).total_seconds(),
            256,
            39,
            51.12,
        ...
        [
            (
                datetime.datetime(2025, 3, 5, 12, 54, 00) - contrail_form_time
            ).total_seconds(),
            254,
            26.1,
            51.09,
        ...
        [
            (
                datetime.datetime(2025, 3, 5, 12, 59, 00) - contrail_form_time
            ).total_seconds(),
            253,
            19.36,
            51.06,
        ...
        [
            (
                datetime.datetime(2025, 3, 5, 13, 4, 0) - contrail_form_time
            ).total_seconds(),
            252.5,
            15.3,
            51.04,
...
plt.scatter(
    kepler_commands[:, 4], kepler_commands[:, 3], c="r", label="Kepler Commands"
)
plt.xlabel("lon")
plt.ylabel("lat")
plt.legend()
